...
def process_audio(audio_fpath):
    # print(audio)
    # print(type(audio))
    audio_fpath = "./test.wav"
    audio_info = sf.info(audio_fpath)
    print(audio_info) # for debug
    data, sr = librosa.load(audio_fpath, sr=16000, dtype=np.float32)
    ...
    for segment in segments:
        ...
        yield current_cool_whisper_transcript, current_whisper_transcript
    end_time = time.time()
    print(f"Total processing time: {end_time -  start_time:.2f}") # calculate the overall processing time
...
with gr.Blocks() as demo:
    ...
    submit_btn.click(fn=process_audio, inputs=audio_input, outputs=[text1, text2])
